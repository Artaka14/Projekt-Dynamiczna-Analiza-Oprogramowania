...
\begin{document}
	...
	Poniżej przedstawiono trzy przykładowe aplikacje z opisem ekranu.
	...
	\subsection{App}
	Główne okno aplikacji, odpowiedzialne za przełączanie ekranów.
	
	\subsection{Screen1}
	Ekran obsługujący wykresy kursu giełdowego.
	
	\subsection{Screen2}
	Obsługuje wykresy danych z Google Trends.
	
	\subsection{Screen3}
	Obsługuje dane z sprawozdań kwartalnych CD Projekt S.A.
	...
	\subsection*{SplashScreen}
	\begin{itemize}
		\item \_\_init\_\_(self) - ustawia wysokość szerokość i zawartość splash screena
		\item center\_x(self, window\_width), center\_y(self, window\_width) - pomocnicze funkcje do ustawienia splash screena na środku ekranu
		\item load\_data\_with\_delay(self) - pobiera dane podczas splash screena i czeka co najmniej 2,5 sekundy
		\item open\_main\_app(self, data) - zamyka splash screen i otwiera główne okno aplikacji
	\end{itemize}
	
	\subsection*{App}
	\begin{itemize}
		\item \_\_init\_\_(self, master, preloaded\_data, preloaded\_trends) - ustawia wysokość szerokość i zawartość głównego okna aplikacji
		\item showFrame(self, frame, text) - zmienia widoczny ekran i nazwę aplikacji
	\end{itemize}
	
	\subsection*{Screen1}
	\begin{itemize}
		\item \_\_init\_\_(self, master, preloaded\_data, preloaded\_trends) - ustawia ramki okna wykresu akcji
		\item showPlot(self, period) - aktualizuje wykres po kliknięciu któregoś z przycisków
		\item showCustomDatePlot(self) - aktualizuje wykres po wpisaniu własnego zakresu
		\item updatePriceLabel(self) - aktualizuje cenę po kliknięciu przycisku
		\item updateMinMaxLabels(self, data) - aktualizuje min i max ceny po kliknięciu przycisku
		\item showError(self, title, message, icon) - pokazuje okienko z błędem
	\end{itemize}
	
	\subsection*{Screen2}
	\begin{itemize}
		\item \_\_init\_\_(self, master, preloaded\_data, preloaded\_trends) - ustawia okna wykresów Google Trends
		\item showTrendsPlot(self, period) - tworzy wykres dla Google Trends z określonego zakresu
		\item refreshTrends(self) - odświeża dane używane do rysowania wykresu Google Trends
	\end{itemize}

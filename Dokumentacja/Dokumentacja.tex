...
\author{Dariusz Kołodziejczyk, Sebastian Bek, Mikołaj Maliszewski}
...
\subsubsection{Cache (test\_cache.py)}

Testy te sprawdzają poprawność działania mechanizmu cache w izolacji od reszty systemu.

\begin{itemize}
	\item \textbf{test\_save\_and\_load\_cache} -- weryfikuje, że zapisany cache jest poprawnie odczytywany bez modyfikacji danych.
	\item \textbf{test\_invalid\_json\_moves\_file} -- symuluje uszkodzony plik JSON i sprawdza, czy zostaje on przeniesiony, a cache zwraca pusty słownik.
	\item \textbf{test\_invalidate\_removes\_entry} -- testuje funkcję unieważniania pojedynczego wpisu cache.
\end{itemize}

\subsubsection{Konwersja danych (test\_df.py)}

Testy dotyczą funkcji odpowiedzialnej za konwersję danych JSON do obiektu \texttt{DataFrame}.

\begin{itemize}
	\item \textbf{test\_df\_from\_entry\_valid} -- poprawna konwersja prawidłowego JSON-a do \texttt{DataFrame}.
	\item \textbf{test\_df\_from\_entry\_invalid\_json} -- obsługa błędnego JSON-a (zwracane \texttt{None}).
	\item \textbf{test\_df\_from\_entry\_empty} -- zachowanie funkcji przy pustym wejściu.
\end{itemize}

\subsection{Testy integracyjne}

\subsubsection{Pobieranie danych trendów (test\_gettrends.py)}

Testy integracyjne sprawdzające współpracę warstwy cache z logiką pobierania danych (z użyciem atrap API).

\begin{itemize}
	\item \textbf{test\_gettrends\_fetches\_and\_saves} -- dane są pobierane z API i zapisywane do cache.
	\item \textbf{test\_gettrends\_uses\_cache} -- przy istniejącym cache API nie jest wywoływane.
	\item \textbf{test\_gettrends\_fallback\_on\_error} -- w przypadku błędu API następuje fallback do cache.
\end{itemize}

\subsection{Testy funkcjonalne (GUI)}

\subsubsection{Ekran trendów (test\_trends\_functionals.py)}

Testy funkcjonalne sprawdzające zachowanie interfejsu użytkownika.

\begin{itemize}
	\item \textbf{test\_trends\_screen\_shows\_plot} -- sprawdza, czy po akcji użytkownika renderowany jest wykres.
	\item \textbf{test\_refresh\_forces\_reload} -- weryfikuje, że odświeżenie wymusza ponowne pobranie danych.
	\item \textbf{test\_trends\_error\_message} -- sprawdza wyświetlenie komunikatu błędu w UI.
\end{itemize}

\subsection{Testy wydajnościowe}

\subsubsection{Wydajność cache (test\_trends\_performance.py)}

Test mierzy czas dostępu do danych zapisanych lokalnie w cache.

\begin{itemize}
	\item \textbf{test\_cache\_read\_is\_fast} -- weryfikuje, że odczyt dużego \texttt{DataFrame} z cache trwa krócej niż 50 ms.
\end{itemize}

    \subsection{Screen poprawności testów}
    \begin{figure}[H]
    	\centering
    	\includegraphics[width=0.7\textwidth]{Obrazy/Ekran4.png}
    	\caption{Wszystkie testy poza jednym przeszły}
    	\label{fig:app1}
    \end{figure}
